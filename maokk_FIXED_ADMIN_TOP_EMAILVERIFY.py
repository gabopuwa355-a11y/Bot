...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "📋 My accounts":
        ...
        cur.execute("""
            ...
            WHERE a.user_id=? AND a.state IN ('shown','done1','waiting_admin','approved','rejected','canceled','timeout')
        """, (user.id,))
...
def admin_top_users(period: str = "daily", limit: int = 50):
    ...
    cur.execute(
        ...
        WHERE a.created_at >= ? AND a.state IN ('shown','done1','waiting_admin','approved','rejected','canceled','timeout')
        GROUP BY a.user_id
        ORDER BY c DESC
        LIMIT ?
        """,
