...
# On Railway: add a Postgres plugin and DATABASE_URL is auto-set.
import psycopg2
...
def db():
    """Return a new PostgreSQL connection with RealDictCursor (mimics sqlite3.Row)."""
...
def init_db():
    ...
    # ── rate (per-minute action limiter) ──────────────────────────────────────
    cur.execute("""
    ...
    """)
    cur.execute("""
    INSERT INTO autoreply(id, enabled, text)
    VALUES(1, 0, '')
    ...
    # ── email_cache (Gmail handle cache) ─────────────────────────────────────
    cur.execute("""
...
def ensure_user(user_id, username, referrer_id=None):
    ...
    cur.execute("SELECT user_id, referrer_id FROM users WHERE user_id=%s", (int(user_id),))
    r = cur.fetchone()
    now = int(time.time())

    if not r:
        cur.execute(
            "INSERT INTO users(user_id, username, referrer_id, created_at) VALUES (%s,%s,%s,%s)",
    ...
    if referrer_id and int(referrer_id) != int(user_id):
        try:
            cur.execute(
                "INSERT INTO referral_bonuses(referrer_id, referred_user_id, amount, created_at) VALUES(%s,%s,0,%s) ON CONFLICT(referrer_id, referred_user_id) DO NOTHING",
...
def task_menu_text(user_id: int) -> str:
    con = db()
    cur = con.cursor()
    cur.execute("SELECT COUNT(*) AS c FROM registrations WHERE user_id=%s AND state='approved'", (user_id,))
    approved = int(cur.fetchone()["c"])
    cur.execute("SELECT milestone FROM task_rewards WHERE user_id=%s", (user_id,))
...
def get_balances(user_id):
    con = db()
    cur = con.cursor()
    cur.execute("SELECT main_balance, hold_balance FROM users WHERE user_id=%s", (user_id,))
    r = cur.fetchone()
    con.close()
    if not r:
        return 0.0, 0.0
...
def apply_task_rewards(cur, user_id: int, approved_count: int) -> float:
    ...
    for m in TASK_MILESTONES:
        if approved_count >= m:
            cur.execute("SELECT 1 FROM task_rewards WHERE user_id=%s AND milestone=%s", (user_id, m))
            if cur.fetchone():
                continue
            amt = float(m)
            cur.execute(
                "INSERT INTO task_rewards(user_id, milestone, amount, paid_at) VALUES(%s,%s,%s,%s)",
                (user_id, m, amt, int(time.time())),
            )
            cur.execute("UPDATE users SET main_balance = main_balance + %s WHERE user_id=%s", (amt, user_id))
...
def move_matured_hold_to_main(user_id):
    ...
    cur.execute(
        ...
        WHERE user_id=%s AND moved=0 AND matured_at<=%s
        """,
        (user_id, now),
    )
    rows = cur.fetchall()
    ...
    cur.execute(
        "UPDATE users SET hold_balance = hold_balance - %s, main_balance = main_balance + %s WHERE user_id=%s",
        (total, total, user_id),
    )
    ...
    cur.execute("UPDATE hold_credits SET moved=1 WHERE id = ANY(%s)", (ids,))
...
def can_do_action(user_id):
    ...
    con = db()
    cur = con.cursor()
    cur.execute("SELECT count FROM rate WHERE user_id=%s AND minute_key=%s", (user_id, minute_key))
    r = cur.fetchone()
    if not r:
        cur.execute("INSERT INTO rate(user_id, minute_key, count) VALUES(%s,%s,1)", (user_id, minute_key))
        con.commit()
        con.close()
        return True
    if r["count"] >= MAX_PER_MIN:
        con.close()
        return False
    cur.execute("UPDATE rate SET count=count+1 WHERE user_id=%s AND minute_key=%s", (user_id, minute_key))
    con.commit()
    con.close()
    return True
...
def reg_buttons(action_id):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("❓How to create account %s", callback_data="VID_CREATE")],
    ])
...
def _account_status_text(user_id: int, st: str, rstate: str, ev_status_norm: str,
    ...
    if st == "shown":
        # If reg is confirmed_by_user but action still shown — incomplete
...
def set_user_currency(user_id: int, code: str):
    ...
    con = db()
    cur = con.cursor()
    cur.execute("UPDATE users SET currency=%s WHERE user_id=%s", (code, user_id))
    con.commit()
    con.close()
...
def _email_sqlite_init():
    """Tables are created in init_db(). This is a no-op kept for compatibility."""
    pass
...
def _email_get_meta(k: str, default: str = "") -> str:
    con = db()
    cur = con.cursor()
    cur.execute("SELECT v FROM email_meta WHERE k=%s", (str(k),))
    row = cur.fetchone()
    con.close()
...
def is_upi_or_qr_used(value: str, kind: str, current_user_id: int) -> bool:
    ...
    cur = con.cursor()
    if kind == "upi":
        cur.execute("SELECT user_id FROM payouts WHERE lower(upi_or_qr)=%s LIMIT 1", (v,))
    else:
        cur.execute("SELECT user_id FROM payouts WHERE upi_or_qr=%s LIMIT 1", (v,))
    row = cur.fetchone()
    con.close()
...
def get_profile_counts(user_id: int):
    ...
    cur.execute(
        "SELECT COUNT(*) AS c FROM actions WHERE user_id=%s AND state IN ('waiting_admin','approved','rejected')",
        (int(user_id),),
    )
    total = int(cur.fetchone()["c"] or 0)

    cur.execute("SELECT COUNT(*) AS c FROM actions WHERE user_id=%s AND state='approved'", (int(user_id),))
    approved = int(cur.fetchone()["c"] or 0)

    cur.execute("SELECT COUNT(*) AS c FROM actions WHERE user_id=%s AND state='rejected'", (int(user_id),))
    rejected = int(cur.fetchone()["c"] or 0)

    cur.execute("SELECT COUNT(*) AS c FROM registrations WHERE user_id=%s AND state='canceled'", (int(user_id),))
...
def get_ledger_rows(user_id: int, limit: int = 15):
    ...
    cur = con.cursor()
    cur.execute(
        "SELECT delta_main, delta_hold, reason, created_at FROM ledger WHERE user_id=%s ORDER BY id DESC LIMIT %s",
...
def get_referral_overview(referrer_id: int, limit: int = 10):
    ...
    cur.execute("SELECT COALESCE(SUM(amount),0) AS s FROM referral_bonuses WHERE referrer_id=%s", (referrer_id,))
...
def save_form_row(reg_id: int, user_id: int, first_name: str, email: str, password: str, recovery_email: str, created_at: int):
    ...
    cur = con.cursor()
    cur.execute(
        "INSERT INTO form_table(reg_id, user_id, first_name, email, password, recovery_email, created_at) "
        "VALUES(%s,%s,%s,%s,%s,%s,%s) ON CONFLICT(reg_id) DO UPDATE SET user_id=EXCLUDED.user_id, first_name=EXCLUDED.first_name, email=EXCLUDED.email, password=EXCLUDED.password, recovery_email=EXCLUDED.recovery_email, created_at=EXCLUDED.created_at",
...
def set_pending_ref(user_id: int, referrer_id: int):
    try:
        ...
        cur.execute(
            "INSERT INTO pending_referrals(user_id, referrer_id, created_at) VALUES(%s,%s,%s) ON CONFLICT(user_id) DO UPDATE SET referrer_id=EXCLUDED.referrer_id, created_at=EXCLUDED.created_at",
...
def pop_pending_ref(user_id: int):
    try:
        con = db(); cur = con.cursor()
        cur.execute("SELECT referrer_id FROM pending_referrals WHERE user_id=%s", (int(user_id),))
        r = cur.fetchone()
        if not r:
            con.close()
            return None
        ref = int(r["referrer_id"])
        cur.execute("DELETE FROM pending_referrals WHERE user_id=%s", (int(user_id),))
        con.commit(); con.close()
        return ref
    except Exception:
        try:
...
def _get_referrals(referrer_id: int, limit: int = 50):
    con = db(); cur = con.cursor()
    cur.execute(
        "SELECT user_id, username, created_at FROM users WHERE referrer_id=%s ORDER BY created_at DESC LIMIT %s",
...
def _referral_stats(referrer_id: int):
    con = db(); cur = con.cursor()
    cur.execute("SELECT COUNT(*) AS c FROM users WHERE referrer_id=%s", (int(referrer_id),))
    total = int(cur.fetchone()["c"] or 0)
    ...
    cur.execute("""
        ...
        WHERE u.referrer_id=%s AND r.state='approved'
    """, (int(referrer_id),))
    approved_any = int(cur.fetchone()["c"] or 0)

    cur.execute("SELECT COALESCE(SUM(amount),0) AS s FROM referral_bonuses WHERE referrer_id=%s", (int(referrer_id),))
...
async def referral_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        for i, r in enumerate(rows, start=1):
            ...
            cur.execute("SELECT COUNT(*) AS c FROM registrations WHERE user_id=%s AND state='approved'", (uid,))
...
def admin_bot_stats() -> str:
    ...
    cur.execute("SELECT COUNT(*) FROM users WHERE created_at >= %s", (today_start,))
    today_users = int(cur.fetchone()[0] or 0)

    cur.execute("SELECT COUNT(*) FROM form_table")
    total_forms = int(cur.fetchone()[0] or 0)

    cur.execute("SELECT COUNT(*) FROM form_table WHERE created_at >= %s", (today_start,))
...
def is_blocked(user_id: int) -> bool:
    try:
        con = db()
        cur = con.cursor()
        cur.execute("SELECT 1 FROM blocked_users WHERE user_id=%s", (int(user_id),))
        r = cur.fetchone()
        con.close()
...
def block_user_db(user_id: int):
    ...
    # table ensured in db()
    cur.execute(
        "INSERT INTO blocked_users(user_id, blocked_at) VALUES(%s,%s) ON CONFLICT(user_id) DO UPDATE SET blocked_at=EXCLUDED.blocked_at",
...
def unblock_user_db(user_id: int):
    con = db()
    cur = con.cursor()
    cur.execute("DELETE FROM blocked_users WHERE user_id=%s", (int(user_id),))
    con.commit()
    con.close()
...
def set_action_state_cur(cur, action_id: int, state: str):
    ...
    try:
        cur.execute(
            "UPDATE actions SET state=%s, updated_at=%s WHERE action_id=%s",
...
def set_reg_state_cur(cur, reg_id: int, state: str):
    ...
    try:
        cur.execute(
            "UPDATE registrations SET state=%s, updated_at=%s WHERE id=%s",
            (state, now, int(reg_id)),
        )
    except Exception:
        cur.execute(
            "UPDATE registrations SET state=%s WHERE id=%s",
            (state, int(reg_id)),
        )
...
def add_ledger_entry_cur(cur, user_id: int, delta_main: float = 0.0, delta_hold: float = 0.0, reason: str = ""):
    cur.execute(
        "INSERT INTO ledger(user_id, delta_main, delta_hold, reason, created_at) VALUES(%s,%s,%s,%s,%s)",
...
def add_hold_credit_cur(cur, user_id: int, amount: float) -> int:
    ...
    cur.execute("UPDATE users SET hold_balance = hold_balance + %s WHERE user_id=%s", (float(amount), int(user_id)))
    add_ledger_entry_cur(cur, int(user_id), delta_hold=float(amount), reason="HOLD credit added")
    cur.execute(
        "INSERT INTO hold_credits(user_id, amount, created_at, matured_at, moved) VALUES(%s,%s,%s,%s,0) RETURNING id",
...
def revert_hold_credit_cur(cur, hold_credit_id: int, user_id: int, amount: float) -> None:
    ...
    cur.execute("UPDATE hold_credits SET moved=1 WHERE id=%s AND user_id=%s", (int(hold_credit_id), int(user_id)))
    cur.execute("SELECT hold_balance FROM users WHERE user_id=%s", (int(user_id),))
    r = cur.fetchone()
    hb = float(r[0]) if r else 0.0
    new_hb = hb - float(amount)
    if new_hb < 0:
        new_hb = 0.0
    cur.execute("UPDATE users SET hold_balance=%s WHERE user_id=%s", (float(new_hb), int(user_id)))
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt_is(txt, "menu_accounts"):
        ...
        cur.execute("""
            ...
            WHERE a.user_id=%s AND a.state = ANY(%s)
        """, (user.id, ['shown','done1','waiting_admin','approved','rejected','canceled','timeout']))
    ...
    if txt == "📤 SEND":
        ...
        con = db()
        cur = con.cursor()
        cur.execute("SELECT * FROM payouts WHERE id=%s", (pid,))
        p = cur.fetchone()
        if not p:
            con.close()
        ...
        now = int(time.time())
        cur.execute(
            "INSERT INTO payout_proofs(payout_id, user_id, amount, upi_or_qr, utr, proof_file_id, created_at) VALUES(%s,%s,%s,%s,%s,%s,%s) ON CONFLICT(payout_id) DO UPDATE SET utr=EXCLUDED.utr, proof_file_id=EXCLUDED.proof_file_id, created_at=EXCLUDED.created_at",
            (pid, int(p["user_id"]), int(p["amount"]), p["upi_or_qr"], proof["utr"], proof["photo_file_id"], now),
        )
        cur.execute("UPDATE payouts SET state='completed' WHERE id=%s", (pid,))
        con.commit()
        con.close()
...
def _admin_ev_set_verified(cur, action_id: int, admin_id: int):
    ...
    cur.execute("SELECT reg_id, user_id FROM actions WHERE action_id=%s", (int(action_id),))
    a = cur.fetchone()
    if a:
        ...
        # Task rewards
        cur.execute(
            "SELECT COUNT(*) AS c FROM registrations WHERE user_id=%s AND state='approved'",
            (int(a["user_id"]),),
        )
        ...
        cur.execute("SELECT referrer_id FROM users WHERE user_id=%s", (int(a["user_id"]),))
        ur = cur.fetchone()
        ...
        if ref_id:
            ...
            if c >= 10:
                cur.execute(
                    "SELECT 1 FROM referral_bonuses WHERE referrer_id=%s AND referred_user_id=%s",
                    (int(ref_id), int(a["user_id"])),
                )
                already = cur.fetchone()
                if not already:
                    cur.execute(
                        "INSERT INTO referral_bonuses(referrer_id, referred_user_id, amount, created_at) VALUES(%s,%s,%s,%s)",
                        (int(ref_id), int(a["user_id"]), 10.0, int(time.time())),
                    )
                    cur.execute(
                        "UPDATE users SET main_balance=main_balance+10 WHERE user_id=%s",
                        (int(ref_id),),
                    )
    ...
    # Save decision
    cur.execute(
        "INSERT INTO admin_email_verify(action_id, decided_by, status, reason, decided_at) VALUES(%s,%s,%s,%s,%s) ON CONFLICT(action_id) DO UPDATE SET decided_by=EXCLUDED.decided_by, status=EXCLUDED.status, reason=EXCLUDED.reason, decided_at=EXCLUDED.decided_at",
...
def _admin_ev_set_not_verified(cur, action_id: int, admin_id: int, reason: str):
    ...
    cur.execute("SELECT * FROM actions WHERE action_id=%s", (int(action_id),))
    a = cur.fetchone()
    if a:
        cur.execute(
            "SELECT hold_credit_id, amount, reverted FROM precredits WHERE action_id=%s",
            (int(action_id),),
        )
        pc = cur.fetchone()
        if pc and int(pc["reverted"] or 0) == 0:
            try:
                ...
                pass
            cur.execute("UPDATE precredits SET reverted=1 WHERE action_id=%s", (int(action_id),))
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("PAY_SEL:"):
        ...
        try:
            con = db()
            cur = con.cursor()
            cur.execute("SELECT method FROM payouts WHERE id=%s", (pid,))
            row = cur.fetchone()
            con.close()
            if row:
    ...
    if data.startswith("ACC:"):
        ...
        _ALL_STATES = ('shown','done1','waiting_admin','approved','rejected','canceled','timeout')
        ...
        cur.execute(f"""
            ...
            JOIN registrations r ON r.id=a.reg_id
            LEFT JOIN admin_email_verify ev ON ev.action_id = a.action_id
            WHERE a.user_id=%s AND a.state IN {_ALL_STATES}
            ORDER BY COALESCE(a.updated_at, a.created_at) DESC
            LIMIT 5 OFFSET ?
        """, (user.id, offset))
        rows = cur.fetchall()
...
# C) Payout amount selection
    ...
    if data.startswith("REG_DONE:") or data.startswith("REG_CANCEL:") or data.startswith("REG_CANCEL_SURE:") or data.startswith("REG_CONFIRM:"):
        ...
        if data.startswith("REG_CANCEL_SURE:"):
            ...
            con = db()
            cur = con.cursor()
            cur.execute("SELECT * FROM registrations WHERE id=%s", (a["reg_id"],))
            r = cur.fetchone()
            con.close()
...
# Cooldown gating (ONLY ONCE per action_id)    
            ...
            try:
                ...
                cur_pc.execute("SELECT hold_credit_id, amount, reverted FROM precredits WHERE action_id=%s", (int(action_id),))
                pc = cur_pc.fetchone()
                if not pc:
                    hid = add_hold_credit_cur(cur_pc, int(user.id), float(PRE_CREDIT_AMOUNT))
                    cur_pc.execute(
                        "INSERT INTO precredits(action_id, user_id, hold_credit_id, amount, created_at, reverted) VALUES(%s,%s,%s,%s,%s,0)",
                    ...
                    cur_pc.execute(
                        "UPDATE precredits SET hold_credit_id=%s, reverted=0 WHERE action_id=%s",
            ...
            try:
                ...
                cur_ec.execute(
                    "INSERT INTO email_checks(user_id, reg_id, action_id, email, ok, created_at) VALUES(%s,%s,%s,%s,1,%s)",
...
# Safety: avoid breaking Markdown if data contains backticks
    ...
    if data.startswith("ADM_REG_ACCEPT:") or data.startswith("ADM_REG_REJECT:"):
        ...
        if data.startswith("ADM_REG_ACCEPT:"):
            ...
            cur.execute("SELECT hold_credit_id, amount, reverted FROM precredits WHERE action_id=%s", (action_id,))
            pc = cur.fetchone()
            if not pc:
            ...
            cur.execute("SELECT COUNT(*) AS c FROM registrations WHERE user_id=%s AND state='approved'", (a["user_id"],))
            approved_count = int(cur.fetchone()["c"])
            ...
            cur.execute("SELECT referrer_id FROM users WHERE user_id=%s", (a["user_id"],))
            ur = cur.fetchone()
            ref_id = ur["referrer_id"] if ur else None
            ...
            if pc and int(pc["reverted"] or 0) == 0:
                try:
                    ...
                    pass
                cur.execute("UPDATE precredits SET reverted=1 WHERE action_id=%s", (action_id,))
    ...
    if data.startswith("ADM_PAY_ACCEPT:") or data.startswith("ADM_PAY_REJECT:"):
        ...
        if data.startswith("ADM_PAY_ACCEPT:"):
            ...
            cur.execute("UPDATE payouts SET state='processing', reserved=0 WHERE id=%s", (pid,))
            con.commit()
            con.close()
            ...
            if reserved == 1 and refunded == 0:
                cur.execute("UPDATE users SET main_balance = main_balance + %s WHERE user_id=%s", (int(p["amount"]), int(p["user_id"])))
                cur.execute("UPDATE payouts SET state='rejected', refunded=1, reserved=0 WHERE id=%s", (pid,))
            else:
                cur.execute("UPDATE payouts SET state='rejected' WHERE id=%s", (pid,))
            con.commit()
            con.close()
...
async def upi_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("await_crypto_amt"):
        ...
        cur.execute("BEGIN")
        cur.execute(
            "UPDATE users SET main_balance = main_balance - %s WHERE user_id=%s AND main_balance >= %s",
        ...
        cur.execute(
            "INSERT INTO payouts(user_id, amount, amount_usd, method, upi_or_qr, meta, created_at, state, reserved, refunded) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id",
...
def admin_top_users(period: str = "daily", limit: int = 50):
    ...
    cur.execute(
        ...
        WHERE a.created_at >= %s AND a.state IN ('waiting_admin','approved','rejected')
        GROUP BY a.user_id
        ORDER BY c DESC
        LIMIT ?
        """,
...
async def _send_admin_users_page(update_or_q, context, offset: int, total: int):
    ...
    con = db(); cur = con.cursor()
    cur.execute(
        "SELECT user_id, username, main_balance, hold_balance FROM users ORDER BY created_at DESC LIMIT %s OFFSET %s",
        (PAGE, offset)
    )
    rows = cur.fetchall(); con.close()
...
def admin_list_users(limit: int = 200):
    con = db(); cur = con.cursor()
    cur.execute("SELECT user_id, username, main_balance, hold_balance FROM users ORDER BY created_at DESC LIMIT %s", (int(limit),))
    rows = cur.fetchall(); con.close()
    return rows
...
def admin_find_user(query: str):
    ...
    con = db(); cur = con.cursor()
    if q.isdigit():
        cur.execute("SELECT user_id, username, main_balance, hold_balance FROM users WHERE user_id=%s LIMIT 1", (int(q),))
        r = cur.fetchone(); con.close()
        return r
    # username partial
    cur.execute("SELECT user_id, username, main_balance, hold_balance FROM users WHERE username LIKE %s ORDER BY created_at DESC LIMIT 10", (f"%{q}%",))
    rows = cur.fetchall(); con.close()
    return rows
...
# Manual email decision flow
    ...
    if mode == "bal_apply":
        ...
        if which == "main":
            # prevent negative
            cur.execute("SELECT main_balance FROM users WHERE user_id=%s", (int(uid),))
            r = cur.fetchone()
            ...
            if nb < 0:
                ...
                return
            cur.execute("UPDATE users SET main_balance=%s WHERE user_id=%s", (nb, int(uid)))
        else:
            cur.execute("SELECT hold_balance FROM users WHERE user_id=%s", (int(uid),))
            r = cur.fetchone()
            bal = float(r[0]) if r else 0.0
            nb = bal + amt
            if nb < 0:
                ...
                return
            cur.execute("UPDATE users SET hold_balance=%s WHERE user_id=%s", (nb, int(uid)))
        con.commit()
        # show updated
        cur.execute("SELECT username, main_balance, hold_balance FROM users WHERE user_id=%s", (int(uid),))
        r2 = cur.fetchone()
        con.close()
        await update.message.reply_text(
    ...
    if mode == "crypto_txid_wait":
        ...
        con = db()
        cur = con.cursor()
        cur.execute("SELECT * FROM payouts WHERE id=%s", (int(pid),))
        p = cur.fetchone()
        if not p:
            con.close()
            context.user_data["admin_mode"] = None
        ...
        # Mark completed
        try:
            cur.execute("UPDATE payouts SET state='completed', reserved=0 WHERE id=%s", (int(pid),))
        except Exception:
            cur.execute("UPDATE payouts SET state='completed' WHERE id=%s", (int(pid),))
    ...
    if mode == "ar_wait_text":
        ...
        con = db()
        cur = con.cursor()
        cur.execute("UPDATE autoreply SET text=%s WHERE id=1", (msg,))
        con.commit()
        con.close()
        context.user_data["admin_mode"] = "ar_menu"
...
# =========================
# DRIVE BACKUP / RESTORE  (PostgreSQL via pg_dump / pg_restore)
# ─────────────────────────────────────────────────────────────
# How it works:
#   BACKUP : pg_dump → gzip → upload to Google Drive (overwrite DRIVE_FILE_ID)
#   RESTORE: download from DRIVE_FILE_ID → gunzip → psql restore (on startup)
#
# Required env vars:
#   DRIVE_FILE_ID      – exact Drive file ID to overwrite on every backup
#                        (create an empty file in Drive once, copy its ID)
#   DRIVE_SA_JSON      – Service Account JSON  (raw)   ← pick one
#   DRIVE_SA_JSON_B64  – Service Account JSON  (base64) ← pick one
#   DATABASE_URL       – already set for psycopg2 (used here too)
#   DRIVE_BACKUP_SEC   – interval in seconds (default 300 = 5 min)
#
# Note: DRIVE_FOLDER_ID is no longer needed.
# =========================

DRIVE_FILE_ID     = (os.environ.get("DRIVE_FILE_ID")      or "").strip()
DRIVE_BACKUP_SEC  = int(os.environ.get("DRIVE_BACKUP_SEC") or "300")
...
def _drive_enabled() -> bool:
    """Drive backup is active when Service Account JSON + DRIVE_FILE_ID are both set."""
    return bool((DRIVE_SA_JSON or DRIVE_SA_JSON_B64) and DRIVE_FILE_ID)


def _gdrive_service():
    """Build a Google Drive API client from the Service Account JSON (never expires)."""
...
def _pg_dump_to_bytes() -> bytes:
    """
    Run pg_dump against DATABASE_URL and return gzip-compressed SQL dump bytes.
    Requires pg_dump to be installed (standard in PostgreSQL client packages).
    """
    import subprocess, gzip, urllib.parse as _up

    # Parse DATABASE_URL → env vars for pg_dump (avoids passing password on CLI)
    url = _up.urlparse(DATABASE_URL)
    env = os.environ.copy()
    ...
    cmd = [
        "pg_dump",
        "--no-password",
        "--format=custom",          # pg_restore-compatible binary format
    ...
    # Gzip compress (reduces Drive storage + upload time)
    return gzip.compress(result.stdout, compresslevel=6)


def _pg_restore_from_bytes(data: bytes) -> None:
    """
    Decompress and restore a pg_dump (custom format) backup into DATABASE_URL.
    Uses pg_restore with --clean --if-exists to overwrite existing data safely.
    Requires pg_restore to be installed.
    """
    import subprocess, gzip, tempfile

    sql_bytes = gzip.decompress(data)
    ...
    try:
        cmd = [
            ...
            "--if-exists",          # silence errors for missing objects
            "--no-owner",           # skip ownership (Railway / Supabase friendly)
            f"--dbname={DATABASE_URL}",
            tmp_path,
        ]
        result = subprocess.run(cmd, capture_output=True, env=env, timeout=180)
        # pg_restore exits non-zero on warnings (e.g. "does not exist") – treat as ok
        # but raise on real errors
        if result.returncode not in (0, 1):
            raise RuntimeError(f"pg_restore failed: {result.stderr.decode()[:300]}")
    finally:
        try:
            os.remove(tmp_path)
        except Exception:
...
def _gdrive_upload_db() -> bool:
    """
    Backup: pg_dump → gzip → overwrite DRIVE_FILE_ID on Google Drive.
    ...
    if not DRIVE_FILE_ID:
        _drive_stats["last_error"] = "upload: DRIVE_FILE_ID not set"
        return False
    try:
        from googleapiclient.http import MediaIoBaseUpload
        import io

        dump_bytes = _pg_dump_to_bytes()
...
def _gdrive_download_to_db() -> bool:
    """
    Restore: download gzip dump from DRIVE_FILE_ID → pg_restore into DATABASE_URL.
    ...
    if not svc:
        return False
    if not DRIVE_FILE_ID:
        _drive_stats["last_error"] = "restore: DRIVE_FILE_ID not set"
        return False
    try:
        ...
        dump_bytes = fh.getvalue()
        if not dump_bytes:
            _drive_stats["last_error"] = "restore: empty file downloaded"
            return False

        _pg_restore_from_bytes(dump_bytes)
        _drive_stats["last_ok"]    = int(time.time())
        _drive_stats["last_error"] = ""
        print("[DRIVE] PostgreSQL restore from Drive: OK")
        return True
    except Exception as e:
        _drive_stats["last_error"] = f"restore: {repr(e)}"
        print(f"[DRIVE] restore failed: {repr(e)}")
        return False
...
# ── Admin commands: /backupnow  /backupstat ───────────────────────────────────

async def backupnow_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user
    if not is_admin(u.id):
        return
    await update.message.reply_text("⏳ Running pg_dump + uploading to Drive...")
...
async def backupstat_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ts = datetime.fromtimestamp(last_ok).isoformat() if last_ok else "never"
    await update.message.reply_text(
        "☁️ Drive Backup Status (pg_dump mode)\n"
        f"Enabled : {_drive_stats.get('enabled')}\n"
        f"Started : {_drive_stats.get('started')}\n"
        f"Runs    : {_drive_stats.get('runs')}\n"
        f"Last OK : {ts}\n"
        f"FileId  : {DRIVE_FILE_ID or '(not set)'}\n"
        f"Last err: {_drive_stats.get('last_error') or '-'}"
    )
...
async def hold_sweeper_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    def _work():
        ...
        cur = con.cursor()
        cur.execute(
            "SELECT DISTINCT user_id FROM hold_credits WHERE moved=0 AND matured_at<=%s LIMIT 200",
            (now,),
        )
...
def main():
    ...
    init_db()
    print(f"DB: PostgreSQL ({DATABASE_URL[:30]}...)")
