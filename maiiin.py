...
def _pg_conn():
    ...
    # mark started + ensure tables exist
    try:
...
async def dbstat_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show email_cache rows count + last few handles (PostgreSQL)."""
    try:
        _pg_email_init()
        # count
        con = _pg_conn()
...
async def syncstat_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show last sync tick + counts + last error."""
    try:
        st = SYNC_STATE.copy()
        msg = (
...
async def _post_init(application: Application):
    ...
    try:
        if os.environ.get("DATABASE_URL") and (os.path.exists("token.json") or os.environ.get("GMAIL_TOKEN_JSON")):
            ...
            asyncio.create_task(_gmail_sync_loop(poll_sec=poll_sec))
            print(f"[SYNC] scheduled gmail sync task (poll_sec={poll_sec})")
        else:
            print("[SYNC] not scheduled (missing DATABASE_URL or GMAIL_TOKEN_JSON/token.json)")
    except Exception as e:
