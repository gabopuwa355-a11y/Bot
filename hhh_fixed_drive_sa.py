...
DRIVE_FILE_ID = (os.environ.get("DRIVE_FILE_ID") or "").strip()  # optional: update/restore this exact file
...
def _drive_enabled() -> bool:
    return bool((DRIVE_SA_JSON or DRIVE_SA_JSON_B64) and (DRIVE_FOLDER_ID or DRIVE_FILE_ID))
...
def _gdrive_download_to_db() -> bool:
    """Restore DB on boot (best-effort). Uses DRIVE_FILE_ID if provided, else finds by name in folder."""
    svc = _gdrive_service()
    if not svc:
        return False
    file_id = DRIVE_FILE_ID or _gdrive_find_file_id_by_name(svc, DRIVE_DB_NAME)
    if not file_id:
        return False
    try:
...
def _gdrive_upload_db() -> bool:
    """Upload/overwrite DB. If DRIVE_FILE_ID is set, updates that file. Otherwise uses name in folder."""
    svc = _gdrive_service()
    if not svc:
        return False
    ...
    try:
        ...
        if file_id:
            ...
            if not DRIVE_FOLDER_ID:
                _drive_stats["last_error"] = "upload: DRIVE_FOLDER_ID missing (needed to create new file)";
                return False
...
async def backupnow_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    hint = ""
    if ok and (not DRIVE_FILE_ID) and fid:
        hint = f"\nSet DRIVE_FILE_ID={fid} to always overwrite the same file."
    await update.message.reply_text(
        f"☁️ Drive backup: {'✅ OK' if ok else '❌ FAIL'}\nDB: {DB}\nFileId: {fid}\nLast error: {_drive_stats.get('last_error','')}{hint}"
    )
