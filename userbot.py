...
# Fix: _do_fetch ab get_response() + get_edit() dono handle karta hai
#      taaki edited bot messages miss na hon aur button kaam kare
# requirements.txt:
#   telethon==1.41.1
#   asyncpg==0.29.0
# ==========================================================
...
# ==========================================================
# NEXT MESSAGE HELPER  —  get_response() + get_edit() dono try karta hai
# ==========================================================

async def _get_next_msg(conv, timeout: int = FETCH_TIMEOUT):
    """
    Bot kabhi kabhi naya message bhejta hai, kabhi kabhi purana edit karta hai.
    Dono cases handle karne ke liye get_response() aur get_edit() parallel
    chalate hain — jo pehle aaye woh return karo.
    """
    tasks = [
        asyncio.create_task(conv.get_response(timeout=timeout)),
        asyncio.create_task(conv.get_edit(timeout=timeout)),
    ]
    try:
        ...
        # Jo pehle complete hui uska result lo
...
# ==========================================================
# CORE FETCH  —  get_response + get_edit dono handle karta hai
# ==========================================================
        ...
        async with client.conversation(SOURCE_BOT, timeout=FETCH_TIMEOUT, exclusive=False) as conv:
            ...
            for step in range(20):
                ...
                if not clicked:
                    ...
                    log.warning(
                        f"[fetch] Step{step+1} — no matching button. "
                        f"Available: {btns}. Text: {(msg.text or '')[:120]}"
                    )
                    return

                await asyncio.sleep(STEP_DELAY)

                # Next message — naya ya edited dono pakdo
                try:
                    msg = await _get_next_msg(conv)
                except asyncio.TimeoutError:
                    log.warning(f"[fetch] Timeout at step {step+1}")
                    return
...
async def _health_server():
    ...
    async def handle(request):
        uptime = int(time.time()) - _health_start_time
        body = (
            f"ok\nuptime={uptime}s\n"
            f"clients={len(clients)}\n"
            f"free={sum(1 for l in locks if not l.locked())}\n"
        )
    ...
    await site.start()
    log.info(f"[health] Listening on :{port}")
...
async def main():
    ...
    if not clients:
        log.error("[main] No authorized clients — exiting")
        return

    log.info(f"[main] {len(clients)} client(s) ready — starting job loop")
