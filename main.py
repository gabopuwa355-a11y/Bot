...
def payout_amounts_with_back_kb() -> InlineKeyboardMarkup:
    """Payout amount buttons + a single back button to payout type chooser."""
    kb = payout_amounts_kb()
    rows = []
    try:
        rows = [list(r) for r in (kb.inline_keyboard or [])]
    except Exception:
        rows = []
    rows.append([InlineKeyboardButton("🔙 BACK", callback_data="PAYOUT_TYPE:MENU")])
    return InlineKeyboardMarkup(rows)

def withdraw_back_kb() -> InlineKeyboardMarkup:
    """Back button used during UPI/CRYPTO text input steps."""
    return InlineKeyboardMarkup([[InlineKeyboardButton("🔙 BACK", callback_data="PAYOUT_TYPE:MENU")]])
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("payout_type_select") and txt in ("1. UPI 🚀", "1. UPI"):
        ...
        # Same flow as PAYOUT_TYPE:UPI callback
        await update.message.reply_text(
    ...
    if context.user_data.get("payout_type_select") and txt.startswith("2. CRYPTO"):
        ...
        bal_usdt = (float(bal_inr) / USDT_RATE) if USDT_RATE else 0.0
        await update.message.reply_text(
            f"""Wallet address like: 0xb2450F5B107b4e04087cB70cDD8E6476385236B1
Blockchain : BEP-20

Available balance: {bal_usdt:.2f} USDT
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "PAYOUT_TYPE:MENU":
        ...
        await q.message.reply_text("CHOOSE TYPE OF WITHDRAWAL", reply_markup=payout_type_kb())
        return
...
# C) Payout amount selection
    if data.startswith("PAY_AMT:"):
        ...
        if float(mainb) < float(amt):
            await q.message.reply_text("BALANCE IS NOT SUFFICIENT FOR WITHDRAWAL 💲", reply_markup=payout_type_kb())
            return
...
async def admin_content_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        user_menu_texts = {'➕ Register a new account', '📋 My accounts', '💰 Balance', '👥 My referrals', '⚙️ Settings', '✅ TASK', '💬 Help'}
        admin_menu_texts = {
            '📢 Broadcast Text','🔗 Broadcast Link','🖼️ Broadcast Image','🖼️ Image + Link','🗃️ Broadcast File','👤 Personal Message',
            '⛔ Block User','✅ Unblock User','💳 PAYOUT REQUEST','📌 Pin Message','📄 Download payout_proofs.pdf',
            '🔝 TOP 50 DAILY USER','🔝 TOP 50 MONTHLY USER','🎭ALL USER','ADD OR DEDUCT BALANCE ♎','EMAIL ✉️ VERIFY','🔙 Back'
        }
        # If admin taps any normal menu button, cancel pending typed-input mode and let later handlers process it.
        if txt in user_menu_texts or txt in admin_menu_texts:
