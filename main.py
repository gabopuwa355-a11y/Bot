...
if not DATABASE_URL:
    raise RuntimeError("DATABASE_URL is missing. Set DATABASE_URL (pooling ON) in Railway/Env variables.")

def _sql_qmark_to_percent(sql: str) -> str:
    """Convert sqlite-style '?' placeholders to psycopg2 '%s'."""
    if not sql:
        return sql
    # Avoid touching literal '?' in strings is complex; this project uses '?' only for placeholders.
...
class _PGConnWrapper:
    ...
    def cursor(self, *args, **kwargs):
        # Always RealDictCursor-like output (dict rows)
...
def db():
    """Create a new Postgres connection (short-lived per operation)."""
    if psycopg2 is None:
        raise RuntimeError("psycopg2 is not installed. Add psycopg2-binary to your requirements.")
    con = psycopg2.connect(DATABASE_URL, connect_timeout=15)
    return _PGConnWrapper(con)

def init_db():
    """Create Postgres schema (idempotent)."""
    con = db()
    cur = con.cursor()
    ...
    # payouts: keep amount as INR integer + amount_usd for crypto display
    cur.execute("""
    ...
    # Gmail handle cache in Postgres
    cur.execute("""
...
def ensure_user(user_id, username, referrer_id=None):
    ...
    if referrer_id and int(referrer_id) != int(user_id):
        try:
            cur.execute(
                "INSERT INTO referral_bonuses(referrer_id, referred_user_id, amount, created_at) VALUES(?,?,0,?) ON CONFLICT (referrer_id, referred_user_id) DO NOTHING",
...
def payout_selected_kb(selected_label: str):
    """Show only selected option + BACK (reply keyboard)."""
    return ReplyKeyboardMarkup(
        [
            [selected_label],
            ["🔙 BACK"],
...
# =========================
# EMAIL SYSTEM (Postgres + Gmail API)
...
def _email_sqlite_init():
    """Legacy name: email cache is stored in Postgres now (created in init_db)."""
    return
...
async def _gmail_sync_loop(poll_sec: int = 5, max_list: int = 200):
    """Poll Gmail every poll_sec seconds, extract gmail handles from message text, store into Postgres."""
    import os
...
def save_form_row(reg_id: int, user_id: int, first_name: str, email: str, password: str, created_at: int):
    ...
    cur = con.cursor()
    cur.execute(
        "INSERT INTO form_table(reg_id, user_id, first_name, email, password, created_at) VALUES(?,?,?,?,?,?) ON CONFLICT (reg_id) DO UPDATE SET user_id=EXCLUDED.user_id, first_name=EXCLUDED.first_name, email=EXCLUDED.email, password=EXCLUDED.password, created_at=EXCLUDED.created_at",
...
def set_pending_ref(user_id: int, referrer_id: int):
    try:
        ...
        cur.execute(
            "INSERT INTO pending_referrals(user_id, referrer_id, created_at) VALUES(?,?,?) ON CONFLICT (user_id) DO UPDATE SET referrer_id=EXCLUDED.referrer_id, created_at=EXCLUDED.created_at",
...
def block_user_db(user_id: int):
    ...
    # table ensured in db()
    cur.execute(
        "INSERT INTO blocked_users(user_id, blocked_at) VALUES(?,?) ON CONFLICT (user_id) DO UPDATE SET blocked_at=EXCLUDED.blocked_at",
...
def _db_write_retry(fn, retries: int = 6, base_sleep: float = 0.15):
    """Retry wrapper for transient Postgres errors (network hiccups, serialization, etc.)."""
    for i in range(int(retries)):
        try:
            return fn()
        except Exception as e:
            msg = str(e).lower()
            # Common transient hints (best-effort)
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "📋 My accounts":
        ...
        if total == 0:
            con.close()
            await update.message.reply_text("Abhi koi account history nahi hai.")
            return
        ...
        for rr in rows:
            ...
            if st in ("rejected", "canceled"):
                ...
                if ev_status_norm in ("NOT_VERIFIED", "REJECTED") and ev_reason.strip():
                    ...
                    status_text = "🔴 Registration canceled"
            elif st in ("waiting_admin", "shown"):
                # Registration still under review / not finished
    ...
    if context.user_data.get("payout_type_select") and context.user_data.get("payout_reply_mode") == "menu" and txt in ("1. UPI 🚀", "1. UPI"):
        ...
        await update.message.reply_text("UPI MODE", reply_markup=back_only_menu())
        return
    ...
    if context.user_data.get("payout_type_select") and context.user_data.get("payout_reply_mode") == "menu" and txt.startswith("2. CRYPTO"):
        ...
        await update.message.  reply_text(f"""Wallet address like: 0xb2450F5B107b4e04087cB70cDD8E6476385236B1
Blockchain : BEP-20

Send your BEP-20 wallet address now:""",reply_markup=back_only_menu())
        return
    ...
    if txt == "📤 SEND":
        ...
        now = int(time.time())
        cur.execute(
            "INSERT INTO payout_proofs(payout_id, user_id, amount, upi_or_qr, utr, proof_file_id, created_at) VALUES(?,?,?,?,?,?,?) ON CONFLICT (payout_id) DO UPDATE SET user_id=EXCLUDED.user_id, amount=EXCLUDED.amount, upi_or_qr=EXCLUDED.upi_or_qr, utr=EXCLUDED.utr, proof_file_id=EXCLUDED.proof_file_id, created_at=EXCLUDED.created_at",
...
def _admin_ev_set_verified(cur, action_id: int, admin_id: int):
    ...
    # Save decision
    cur.execute(
        "INSERT INTO admin_email_verify(action_id, decided_by, status, reason, decided_at) VALUES(?,?,?,?,?) ON CONFLICT (action_id) DO UPDATE SET decided_by=EXCLUDED.decided_by, status=EXCLUDED.status, reason=EXCLUDED.reason, decided_at=EXCLUDED.decided_at",
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ACC:"):
        ...
        if total == 0:
            con.close()
            await q.edit_message_text("Abhi koi account history nahi hai.")
            return
...
async def upi_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("await_crypto_addr"):
        ...
        mainb, _holdb = get_balances(user.id)
        await update.message.reply_text(
            f"✅ Address saved.\n\nAvailable balance: {inr_to_usd_fixed(float(mainb)):.2f} USD\n\n"
    ...
    if context.user_data.get("await_crypto_amt"):
        ...
        if amt_d is None or amt_d.is_nan() or amt_d <= 0:
            await update.message.reply_text(
                "❌ Please enter a valid amount number (USD).",
                reply_markup=back_only_menu(),
            )
            return
        ...
        if not is_valid_bep20_address(addr):
            ...
            context.user_data["await_crypto_amt"] = False
            context.user_data["await_crypto_addr"] = True
            await update.message.reply_text("❌ Address missing. Please send your wallet address again:", reply_markup=back_only_menu())
            return
        ...
        try:
            await update.message.reply_text(
                "✅ YOUR CRYPTO PAYOUT REQUEST IS SENT 📤 TO MY PAYMENT DEPARTMENT 🏬",
                reply_markup=MAIN_MENU
            )
        except Exception:
            pass
...
async def admin_content_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Neon/Postgres restore flow (admin)
    if mode == "restoredb_confirm":
        msg = (update.message.text or "").strip()
        if msg != "CONFIRM RESTORE":
            context.user_data["admin_mode"] = None
            await update.message.reply_text("❌ Restore cancelled.", reply_markup=ADMIN_MENU_KB)
            return

        context.user_data["admin_mode"] = None
        await update.message.reply_text("⏳ Restoring Neon/Postgres from Google Drive latest backup...")

        ok = await asyncio.to_thread(_restore_from_drive_latest_pg)
        if ok:
            await update.message.reply_text("✅ RESTORE DONE. Database has been overwritten from Drive backup.", reply_markup=ADMIN_MENU_KB)
        else:
            await update.message.reply_text(f"❌ RESTORE FAILED\n{_drive_pg_stats.get('last_error','')}", reply_markup=ADMIN_MENU_KB)
        return
...
# =========================
# DRIVE BACKUP / RESTORE (POSTGRES / NEON)
# - Creates pg_dump (custom .dump) and uploads to Google Drive every 2 days
# - Admin-only: /restoredb restores latest dump from Drive into DATABASE_URL (Neon)
# - Optional: /pgbackupnow /pgbackupstat
# =========================

POSTGRES_DATABASE_URL = (os.environ.get("PG_DUMP_URL") or os.environ.get("DATABASE_URL") or os.environ.get("POSTGRES_URL") or "").strip()
DRIVE_PG_BACKUP_PREFIX = (os.environ.get("DRIVE_PG_BACKUP_PREFIX") or "pg_backup").strip()
DRIVE_PG_BACKUP_SEC = int(os.environ.get("DRIVE_PG_BACKUP_SEC") or "172800")  # 2 days default
...
def _gdrive_upload_local_file(local_path: str, drive_name: str) -> bool:
    """Upload any local file to Drive (overwrite by name in DRIVE_FOLDER_ID)."""
    svc = _gdrive_service()
    if not svc:
        return False
    if not os.path.exists(local_path):
        _drive_pg_stats["last_error"] = f"upload: file not found {local_path}"
        return False
    try:
        from googleapiclient.http import MediaFileUpload
        file_id = _gdrive_find_file_id(svc, drive_name)
        ...
        if file_id:
            ...
            _drive_pg_stats["last_uploaded_file_id"] = upd.get("id", "")
        else:
            ...
            _drive_pg_stats["last_uploaded_file_id"] = created.get("id", "")
        _drive_pg_stats["last_ok"] = int(time.time())
        _drive_pg_stats["last_error"] = ""
        return True
    except Exception as e:
        _drive_pg_stats["last_error"] = f"upload: {repr(e)}"
        return False

def _gdrive_find_latest_file_id_by_prefix(prefix: str) -> str:
    """Find newest file in DRIVE_FOLDER_ID whose name contains prefix."""
    svc = _gdrive_service()
    if not svc:
        return ""
    try:
        safe_prefix = (prefix or "").replace("'", "''")
        q = f"'{DRIVE_FOLDER_ID}' in parents and trashed=false and name contains '{safe_prefix}'"
        res = svc.files().list(
            q=q,
            spaces="drive",
        ...
        _drive_pg_stats["last_error"] = f"find_latest: {repr(e)}"
        return ""

def _gdrive_download_file_by_id(file_id: str, out_path: str) -> bool:
    svc = _gdrive_service()
    if not svc:
        return False
    try:
        ...
        _drive_pg_stats["last_error"] = f"download: {repr(e)}"
        return False

def _pg_dump_to_file(out_path: str) -> bool:
    """Create a pg_dump (custom format) from POSTGRES_DATABASE_URL into out_path."""
    if not POSTGRES_DATABASE_URL or not _is_postgres_url(POSTGRES_DATABASE_URL):
        _drive_pg_stats["last_error"] = "pg_dump: missing/invalid DATABASE_URL"
        return False
    try:
        import subprocess
        # -Fc custom format dump
        cmd = ["pg_dump", "-Fc", POSTGRES_DATABASE_URL]
        with open(out_path, "wb") as f:
            p = subprocess.run(cmd, stdout=f, stderr=subprocess.PIPE, timeout=300)
        if p.returncode != 0:
            _drive_pg_stats["last_error"] = "pg_dump error: " + (p.stderr.decode(errors="ignore")[:400])
            return False
        return True
    except Exception as e:
        _drive_pg_stats["last_error"] = f"pg_dump ex: {repr(e)}"
        return False

def _pg_restore_from_file(dump_path: str) -> bool:
    """Restore a custom-format dump into POSTGRES_DATABASE_URL (OVERWRITES)."""
    if not POSTGRES_DATABASE_URL or not _is_postgres_url(POSTGRES_DATABASE_URL):
        _drive_pg_stats["last_error"] = "pg_restore: missing/invalid DATABASE_URL"
        return False
    if not os.path.exists(dump_path):
        _drive_pg_stats["last_error"] = f"pg_restore: dump not found {dump_path}"
        return False
    try:
        import subprocess
        # --clean and --if-exists to overwrite existing objects
        cmd = ["pg_restore", "--clean", "--if-exists", "-d", POSTGRES_DATABASE_URL, dump_path]
        p = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=600)
        if p.returncode != 0:
            _drive_pg_stats["last_error"] = "pg_restore error: " + (p.stderr.decode(errors="ignore")[:500])
            return False
        return True
    except Exception as e:
        _drive_pg_stats["last_error"] = f"pg_restore ex: {repr(e)}"
        return False

async def _drive_pg_backup_loop():
    """Every DRIVE_PG_BACKUP_SEC seconds: pg_dump -> upload to Drive."""
    _drive_pg_stats["started"] = True
    while True:
        try:
            await asyncio.sleep(max(3600, int(DRIVE_PG_BACKUP_SEC)))  # safety min 1h
            _drive_pg_stats["runs"] += 1

            dump_name = f"{DRIVE_PG_BACKUP_PREFIX}_{int(time.time())}.dump"
            ok = await asyncio.to_thread(_pg_dump_to_file, dump_name)
            if not ok:
                continue

            _drive_pg_stats["last_dump_name"] = dump_name
            await asyncio.to_thread(_gdrive_upload_local_file, dump_name, dump_name)

        except Exception as e:
            _drive_pg_stats["last_error"] = f"loop: {repr(e)}"

async def pgbackupnow_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user
    if not is_admin(u.id):
        return
    dump_name = f"{DRIVE_PG_BACKUP_PREFIX}_{int(time.time())}.dump"
    ok_dump = await asyncio.to_thread(_pg_dump_to_file, dump_name)
    ok_up = False
    if ok_dump:
        ok_up = await asyncio.to_thread(_gdrive_upload_local_file, dump_name, dump_name)
    await update.message.reply_text(
        f"☁️ PG Drive backup: {'✅ OK' if (ok_dump and ok_up) else '❌ FAIL'}\n"
        f"Dump: {dump_name if ok_dump else '-'}\n"
        f"Last error: {_drive_pg_stats.get('last_error','')}"
    )

async def pgbackupstat_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user
    if not is_admin(u.id):
        return
    last_ok = _drive_pg_stats.get("last_ok", 0)
    ts = datetime.datetime.fromtimestamp(last_ok).isoformat() if last_ok else "0"
    await update.message.reply_text(
        "☁️ PG Drive Backup Status\n"
        f"Enabled: {_drive_pg_stats.get('enabled')}\n"
        f"Started: {_drive_pg_stats.get('started')}\n"
        f"Runs: {_drive_pg_stats.get('runs')}\n"
        f"Last OK: {ts}\n"
        f"Last dump: {_drive_pg_stats.get('last_dump_name')}\n"
        f"Last error: {_drive_pg_stats.get('last_error')}\n"
        f"FolderId: {DRIVE_FOLDER_ID}"
    )

async def restoredb_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only: restore Neon/Postgres DB from latest Drive dump (2-step confirm)."""
    u = update.effective_user
    if not is_admin(u.id):
        return

    # Step-1: request confirmation
    context.user_data["admin_mode"] = "restoredb_confirm"
    await update.message.reply_text(
        "⚠️ This will OVERWRITE your Neon/Postgres database from the latest Google Drive backup.\n"
        "Type exactly: CONFIRM RESTORE"
    )

def _restore_from_drive_latest_pg() -> bool:
    # Find latest dump in folder and restore it
    if not POSTGRES_DATABASE_URL or not _is_postgres_url(POSTGRES_DATABASE_URL):
        _drive_pg_stats["last_error"] = "restore: missing/invalid DATABASE_URL"
        return False
    fid = _gdrive_find_latest_file_id_by_prefix(DRIVE_PG_BACKUP_PREFIX)
    if not fid:
        _drive_pg_stats["last_error"] = "restore: no pg_backup file found in Drive folder"
        return False
    local_dump = "pg_backup_latest.dump"
    if not _gdrive_download_file_by_id(fid, local_dump):
        return False
    return _pg_restore_from_file(local_dump)
...
async def dbstat_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only: show email_cache rows count + last few handles (Postgres)."""
...
async def _post_init(application: Application):
    ...
    try:
        if DRIVE_FOLDER_ID and DRIVE_TOKEN_JSON:
            ...
            # Postgres/Neon pg_dump backup (every 2 days by default)
            if POSTGRES_DATABASE_URL and _is_postgres_url(POSTGRES_DATABASE_URL):
                _drive_pg_stats['enabled'] = True
                application.job_queue.run_once(lambda *_: asyncio.create_task(_drive_pg_backup_loop()), when=2)
...
def main():
    init_db()
    print("DB: Postgres (Neon) via DATABASE_URL")
    ...
    # Admin-only Neon/Postgres backup/restore commands
    app.add_handler(CommandHandler("pgbackupnow", pgbackupnow_cmd))
    app.add_handler(CommandHandler("pgbackupstat", pgbackupstat_cmd))
