...
# =========================
# PAYOUT TYPE (UPI / CRYPTO) MENUS
# =========================

def payout_type_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
...
def withdraw_back_kb() -> InlineKeyboardMarkup:
    # Back to payout type chooser (NOT main menu)
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔙 BACK", callback_data="PAYOUT_TYPE:MENU")]
    ])

def payout_amounts_with_back_kb() -> InlineKeyboardMarkup:
    kb = payout_amounts_kb().inline_keyboard
    kb2 = kb + [[InlineKeyboardButton("🔙 BACK", callback_data="PAYOUT_TYPE:MENU")]]
    return InlineKeyboardMarkup(kb2)

BEP20_ADDR_RE = re.compile(r"^0x[a-fA-F0-9]{40}$")

def is_valid_bep20_address(addr: str) -> bool:
    return bool(BEP20_ADDR_RE.fullmatch((addr or "").strip()))
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for rr in rows:
            ...
            if status == "rejected":
                try:
                    cur2 = con.cursor()
                    cur2.execute(
                        "SELECT reason FROM admin_email_verify WHERE reg_id=? AND action='reject' ORDER BY id DESC LIMIT 1",
                        (rr["reg_id"],),
                    )
                    row2 = cur2.fetchone()
                    if row2 and row2[0]:
                        reason = str(row2[0]).strip()
                except Exception:
                    reason = None
    ...
    if txt == "💰 Balance":
        ...
        if cur_code and cur_code != "INR":
            ...
            await update.message.reply_text(
                f"MAIN BALANCE= ₹{mainb:.2f} (≈ {fmt_money(main_conv, cur_code)})\n"
                f"HOLD BALANCE= ₹{holdb:.2f} (≈ {fmt_money(hold_conv, cur_code)})",
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith(f"{ADMIN_EV_CB_REASON}:"):
        ...
        rowu = cur.fetchone()
        target_user = int(rowu["user_id"]) if rowu and rowu.get("user_id") is not None else None
        reg_id = int(rowu["reg_id"]) if rowu and rowu.get("reg_id") is not None else None
...
# - Auto-backup to Drive every N seconds (default 300 = 5 min)
