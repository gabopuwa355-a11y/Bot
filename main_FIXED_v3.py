...
Available balance: {bal_usd:.2f} USD
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for rr in rows:
            ...
            if rr["astate"] in ("waiting_admin", "approved"):
                try:
                    ...
                    extra = ""
            lines.append(f"• {rr['email']}  —  {status}  —  {fmt_ts(rr['stime'])}{extra}")
    ...
    if data == "PAYOUT_TYPE:CRYPTO":
        ...
        await q.message.reply_text(
            ...
            "Blockchain : BEP-20\n\n"
            f"Available balance: {inr_to_usd(float(mainb)):.2f} USD\n\n"
...
# C) Payout amount selection
    if data.startswith("PAY_AMT:"):
        ...
        await q.message.reply_text("PLEASE ENTER YOUR UPI ID OR QR CODE", reply_markup=payout_selected_kb("2. CRYPTO ( USDT BEP-20)"))
        return
...
async def upi_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("await_crypto_amt"):
        ...
        amt = int(raw_amt)
        if amt <= 0:
            await update.message.reply_text("❌ Amount must be greater than 0.", reply_markup=payout_selected_kb("2. CRYPTO ( USDT BEP-20)"))
            return
    ...
    if kind == "upi":
        if not (upi.lower().startswith("upi://") or is_valid_upi_id(upi)):
            await update.message.reply_text("❌ INVALID UPI ID. Example: name@bank", reply_markup=payout_selected_kb("2. CRYPTO ( USDT BEP-20)"))
            return
        if is_upi_or_qr_used(upi, "upi", user.id):
            await update.message.reply_text("❌ THIS UPI ID IS ALREADY USED. Please enter a different UPI ID.", reply_markup=payout_selected_kb("2. CRYPTO ( USDT BEP-20)"))
            return
    else:
        if len(upi) < 10:
            await update.message.reply_text("❌ INVALID QR CODE", reply_markup=payout_selected_kb("2. CRYPTO ( USDT BEP-20)"))
            return
        if is_upi_or_qr_used(upi, "qr", user.id):
            await update.message.reply_text("❌ THIS QR CODE IS ALREADY USED. Please send a different QR.", reply_markup=payout_selected_kb("2. CRYPTO ( USDT BEP-20)"))
            return
