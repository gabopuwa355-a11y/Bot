...
def _pg_conn():
    ...
    if not dburl:
        # Don't crash the whole bot if DB isn't wired yet.
        raise RuntimeError(
            "DATABASE_URL missing in Railway Variables. "
            "Set DATABASE_URL = {{ Postgres.DATABASE_URL }} in the WEB service."
        )
...
def _pg_email_handle_exists(handle: str) -> bool:
    ...
    try:
        ...
        # If DB isn't ready/misconfigured, treat as "not found" instead of crashing callbacks.
        return False
...
def main():
    ...
    # Init PG tables early (prevents "relation email_cache does not exist")
    try:
        if os.environ.get("DATABASE_URL"):
            _pg_email_init()
    except Exception as e:
        print("⚠️ PG init skipped:", e)
