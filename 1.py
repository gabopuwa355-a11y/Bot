...
# =========================
# PAYOUT TYPE (REPLY MENU) + HELPERS
# =========================

def get_user_balance(user_id: int) -> float:
    """Return MAIN balance in INR (used for CRYPTO available balance conversion)."""
    try:
        mainb, _holdb = get_balances(int(user_id))
        return float(mainb)
    except Exception:
        return 0.0

def withdraw_back_kb():
    # Back button while user is entering UPI / wallet / amount
    return ReplyKeyboardMarkup([["🔙 Back"]], resize_keyboard=True)

def payout_amounts_with_back_kb() -> InlineKeyboardMarkup:
    """Amount selector (inline) + back-to-type chooser.
    Uses callback_data handled in callbacks(): PAYOUT_TYPE:MENU
    """
    base = payout_amounts_kb().inline_keyboard or []
    kb = [list(row) for row in base]
    kb.append([InlineKeyboardButton("🔙 BACK", callback_data="PAYOUT_TYPE:MENU")])
