...
def payout_menu_crypto_only_kb():
    return ReplyKeyboardMarkup(
        [
            ["2. CRYPTO ( USDT BEP-20)"],
            ["🔙 BACK"],
...
def withdraw_back_kb():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔙 BACK", callback_data="PAYOUT_TYPE:BACK_MENU")]
    ])

def payout_amounts_with_back_kb():
    base = payout_amounts_kb().inline_keyboard or []
    rows = [list(r) for r in base]
    rows.append([InlineKeyboardButton("🔙 BACK", callback_data="PAYOUT_TYPE:BACK_MENU")])
    return InlineKeyboardMarkup(rows)
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "📋 My accounts":
        ...
        cur.execute("""
            ...
            WHERE a.user_id=? AND a.state IN ('shown','done1','waiting_admin','approved','rejected','canceled','processing','created')
        """, (user.id,))
    ...
    if context.user_data.get("payout_type_select") and txt in ("1. UPI 🚀", "1. UPI"):
        context.user_data["payout_selected_mode"] = "upi"
        ...
        await update.message.reply_text("UPI 🚀 selected", reply_markup=payout_menu_upi_only_kb())
        await update.message.reply_text(
            "CHOOSE AMOUNT\n10% FEES IS APPLICABLE",
            reply_markup=payout_amounts_with_back_kb()
        )
        return

    if context.user_data.get("payout_type_select") and txt.startswith("2. CRYPTO"):
        context.user_data["payout_selected_mode"] = "crypto"
        context.user_data.pop("payout_type_select", None)
        context.user_data["await_upi"] = False
        ...
        await update.message.reply_text("CRYPTO ( USDT BEP-20 ) selected", reply_markup=payout_menu_crypto_only_kb())
        await update.message.reply_text(
            f"""Wallet address like: 0xb2450F5B107b4e04087cB70cDD8E6476385236B1
Blockchain : BEP-20

Available balance: {float(mainb):.2f} USD
...
Now send your wallet address:""",
    ...
    if txt in ("🔙 BACK", "🔙 Back", "⬅️ BACK", "⬅ BACK"):
        # Back from payout type menu states
        if context.user_data.get("payout_type_select"):
            context.user_data.pop("payout_type_select", None)
            context.user_data.pop("payout_selected_mode", None)
            await update.message.reply_text("💰 Balance", reply_markup=balance_menu())
            return
        if context.user_data.get("payout_selected_mode") in ("upi", "crypto"):
            context.user_data["await_upi"] = False
            context.user_data["await_crypto_addr"] = False
            context.user_data["await_crypto_amt"] = False
            context.user_data["crypto_addr"] = ""
            context.user_data["payout_selected_mode"] = None
            context.user_data["payout_type_select"] = True
    ...
    if txt == "SUBMIT THE PAYMENT PROOF 🧾":
        ...
        if not pid:
            ...
            return
        con = db(); cur = con.cursor()
        cur.execute("SELECT upi_or_qr FROM payouts WHERE id=?", (pid,))
        prow = cur.fetchone()
        con.close()
        dest = (prow["upi_or_qr"] if prow else "") or ""
        if str(dest).strip().startswith("0x"):
            context.user_data["admin_mode"] = "pay_proof_wait_txid"
            await update.message.reply_text("Send CRYPTO Transaction ID (text).")
        else:
    ...
    if txt == "📤 SEND":
        ...
        cur.execute(
            ...
            (pid, int(p["user_id"]), int(p["amount"]), p["upi_or_qr"], proof["utr"], proof.get("photo_file_id"), now),
        )
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "PAYOUT_TYPE:UPI":
        context.user_data["payout_selected_mode"] = "upi"
        context.user_data.pop("payout_type_select", None)
        await q.message.reply_text("UPI 🚀 selected", reply_markup=payout_menu_upi_only_kb())
        await q.message.reply_text(
            "CHOOSE AMOUNT\n10% FEES IS APPLICABLE",
            reply_markup=payout_amounts_with_back_kb()
        )
    ...
    if data == "PAYOUT_TYPE:CRYPTO":
        ...
        context.user_data["crypto_addr"] = ""
        mainb, _holdb = get_balances(user.id)
        await q.message.reply_text("CRYPTO ( USDT BEP-20 ) selected", reply_markup=payout_menu_crypto_only_kb())
        await q.message.reply_text(
            "Wallet address like: 0xb2450F5B107b4e04087cB70cDD8E6476385236B1\n"
            "Blockchain : BEP-20\n\n"
            f"Available balance: {float(mainb):.2f} USD\n\n"
            "Now send your wallet address:",
            reply_markup=withdraw_back_kb()
        )
        return
    ...
    if data.startswith("PAY_AMT:"):
        ...
        if amt not in allowed:
            await q.message.reply_text("Invalid amount.", reply_markup=payout_amounts_with_back_kb())
            return

        mainb, _holdb = get_balances(user.id)
        if float(mainb) < float(amt):
            await q.message.reply_text("BALANCE IS NOT SUFFICIENT FOR WITHDRAWAL 💲", reply_markup=payout_menu_upi_only_kb())
            return

        context.user_data["await_upi"] = True
        context.user_data["payout_amt"] = amt
        context.user_data["payout_selected_mode"] = "upi"
        await q.message.reply_text("PLEASE ENTER YOUR UPI ID OR QR CODE", reply_markup=withdraw_back_kb())
        return
...
async def upi_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Back handling for payout flows (reply keyboard)
    if txt_in in ("🔙 BACK", "🔙 Back", "⬅️ BACK", "⬅ BACK"):
        if context.user_data.get("await_crypto_addr") or context.user_data.get("await_crypto_amt") or context.user_data.get("await_upi") or context.user_data.get("payout_selected_mode"):
            context.user_data["await_crypto_addr"] = False
            context.user_data["await_crypto_amt"] = False
            context.user_data["await_upi"] = False
            context.user_data["crypto_addr"] = ""
    ...
    if context.user_data.get("await_crypto_addr"):
        ...
        # Allow user to go back by pressing the inline 🔙 BACK button (callback),
        # but if user types "back" we keep them here (no menu jump).
    ...
    if context.user_data.get("await_crypto_amt"):
        ...
        if not raw_amt.isdigit():
            await update.message.reply_text(
                "❌ Please enter a valid amount number (USD). Example: 55",
                reply_markup=withdraw_back_kb(),
            )
            return
        ...
        amt = int(raw_amt)
        if amt <= 0:
            await update.message.reply_text("❌ Amount must be greater than 0.", reply_markup=withdraw_back_kb())
            return

        # Check balance again (safety)
        mainb, _holdb = get_balances(user.id)
        if float(mainb) < float(amt):
            await update.message.reply_text("BALANCE IS NOT SUFFICIENT FOR WITHDRAWAL 💲", reply_markup=withdraw_back_kb())
            return
        ...
        if not is_valid_bep20_address(addr):
            ...
            context.user_data["await_crypto_amt"] = False
            context.user_data["await_crypto_addr"] = True
            await update.message.reply_text("❌ Address missing. Please send your wallet address again:", reply_markup=withdraw_back_kb())
            return
        ...
        cur.execute(
            ...
            (user.id, amt, meta, now, "pending", 1, 0),
        )
        pid = cur.lastrowid
        con.commit()
        con.close()
    ...
    if amt not in (55,110,210,310,510,1050):
        await update.message.reply_text("Invalid amount.", reply_markup=payout_amounts_with_back_kb())
        return
    ...
    if kind == "upi":
        if not (upi.lower().startswith("upi://") or is_valid_upi_id(upi)):
            await update.message.reply_text("❌ INVALID UPI ID. Example: name@bank", reply_markup=withdraw_back_kb())
            return
        if is_upi_or_qr_used(upi, "upi", user.id):
            await update.message.reply_text("❌ THIS UPI ID IS ALREADY USED. Please enter a different UPI ID.", reply_markup=withdraw_back_kb())
            return
    else:
        if len(upi) < 10:
            await update.message.reply_text("❌ INVALID QR CODE", reply_markup=withdraw_back_kb())
            return
        if is_upi_or_qr_used(upi, "qr", user.id):
            await update.message.reply_text("❌ THIS QR CODE IS ALREADY USED. Please send a different QR.", reply_markup=withdraw_back_kb())
            return
...
def admin_top_users(period: str = "daily", limit: int = 50):
    ...
    cur.execute(
        ...
        WHERE a.created_at >= ? AND a.state IN ('shown','done1','waiting_admin','approved','rejected','canceled','processing','created')
        GROUP BY a.user_id
        ORDER BY c DESC
        LIMIT ?
        """,
...
async def admin_content_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "pay_proof_wait_txid":
        ...
        if len(txid) < 10:
            await update.message.reply_text("❌ Send valid Transaction ID (text).")
            return
        context.user_data.setdefault("pay_proof", {})[pid] = {"photo_file_id": None, "utr": txid}
        context.user_data["admin_mode"] = None
        await update.message.reply_text("✅ Transaction ID saved. Now press 📤 SEND.", reply_markup=PAYOUT_SUBMENU_KB)
        return
