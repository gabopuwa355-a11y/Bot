...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith(f"{ADMIN_EV_CB_REASON}:"):
        ...
        # notify user with reason + deduct amount (include same email sent to admin)
        if target_user:
            try:
                safe_email = (str(email_addr).replace("`", "'") if email_addr else "")
                prefix = f"📧 Email: `{safe_email}`\n\n" if safe_email else ""
                msg = (
                    prefix
                    + "WRONG PASSWORD\n\n"
                    + f"REASON= {reason}\n"
...
GDRIVE_SA_JSON = (os.environ.get("GDRIVE_SA_JSON") or "").strip()
...
def _gdrive_service():
    # Uses Service Account JSON from env var (recommended)
    # Folder must be shared with the service account email.
    if not (DRIVE_FOLDER_ID and GDRIVE_SA_JSON):
        return None
    try:
        import json as _json
        from google.oauth2 import service_account
        from googleapiclient.discovery import build as _build
        info = _json.loads(GDRIVE_SA_JSON)
...
async def _post_init(application: Application):
    ...
    try:
        if DRIVE_FOLDER_ID and GDRIVE_SA_JSON:
