...
def _build_account_lines(rows, user_id: int, now_ts: int) -> list:
    ...
    lines = []
    print(f"[ACCOUNTS] _build_account_lines called, rows count: {len(rows)}", flush=True)
    for i, rr in enumerate(rows, 1):
        try:
            print(f"[ACCOUNTS] row {i} keys: {list(rr.keys())}", flush=True)
            ...
            lines.append(line)
            print(f"[ACCOUNTS] row {i} appended OK: {email_display}", flush=True)
        except Exception as e:
            import traceback
            err = traceback.format_exc()
            print(f"[ACCOUNTS] row {i} ERROR: {e}\n{err}", flush=True)
            lines.append(f"━━━━━━━━━━━━━━━━━━\n[row {i} error: {e}]")
    print(f"[ACCOUNTS] returning {len(lines)} lines", flush=True)
    return lines
...
async def user_in_required_channels(context: ContextTypes.DEFAULT_TYPE, user_id: int) -> bool:
    ...
    for chat_username, _url in REQUIRED_CHANNELS:
        try:
            ...
            # Bot is not admin or API error — do not block user
            return True
    return True
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt_is(txt, "menu_accounts"):
        ...
        print(f"[ACCOUNTS-MENU] user={user.id} tapped My Accounts", flush=True)
        con = db()
        cur = con.cursor()
        try:
            ...
            print(f"[ACCOUNTS-MENU] user={user.id} raw actions count (no filter)={raw_total}", flush=True)
            ...
            for aa in all_actions:
                print(f"[ACCOUNTS-MENU] action_id={aa['action_id']} state={aa['state']} reg_id={aa['reg_id']}", flush=True)
            ...
            print(f"[ACCOUNTS-MENU] user={user.id} filtered total={total}", flush=True)
            ...
            """, (user.id,))
            rows = cur.fetchall()
            print(f"[ACCOUNTS-MENU] user={user.id} rows returned={len(rows)}", flush=True)
            con.commit()
        except Exception as e:
            try: con.rollback()
            except Exception: pass
            print(f"[ACCOUNTS-MENU] DB error: {e!r}", flush=True)
            rows = []
            total = 0
        finally:
            try: con.close()
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ACC:"):
        ...
        try:
            ...
            print(f"[ACCOUNTS] DB error: {e!r}", flush=True)
            ...
            if ref_id:
                ...
                if commission_pct > 0:
                    ...
                    if commission_amt > 0:
                        cur.execute(
                            "INSERT INTO referral_bonuses(referrer_id, referred_user_id, amount, created_at) VALUES(?,?,?,?)",
                            (int(ref_id), int(a["user_id"]), float(commission_amt), int(time.time())),
                        )
                        cur.execute(
...
async def setcommission_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        ...
        await update.message.reply_text(
            ...
            f"Per approval earn: ₹{per} (on ₹{PRE_CREDIT_AMOUNT} base)\n\n"
...
def main():
    ...
    # NOTE: lang migration removed — it was wiping lang='hi' to NULL on every restart,
    # causing all Hindi users to lose their language setting after each deployment.
    ...
    print("[VERSION] main.py = v4-reg-updated-at-fix-2026-05-12", flush=True)
