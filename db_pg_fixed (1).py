...
def db():
    ...
    # No connection yet, or it was explicitly closed
...
_TABLE_PK = {
    # Tables whose PK column is named something other than 'id'
    # Every table with INSERT anywhere in the code must be listed here.
    # If missing, shim appends RETURNING id → UndefinedColumn crash.
    "actions":              "action_id",
    "users":                "user_id",
    ...
    "task_rewards":         "user_id",   # composite PK — RETURNING first col only
    "rate":                 "user_id",   # composite PK — RETURNING first col only
    ...
    "email_meta":           "k",
    "admin_settings":       "key",
    # Tables whose PK is 'id' (default — listed for documentation only)
    "registrations":        "id",
    "hold_credits":         "id",
    "ledger":               "id",
    "payouts":              "id",
...
class PgCursorWrapper:
    ...
    def execute(self, sql, params=None):
        ...
        # Is this a DDL statement? Used to decide which errors to swallow vs re-raise.
        _is_ddl = bool(re.match(r"\s*(ALTER|CREATE|DROP)\b", translated, re.IGNORECASE))

        try:
            self._cur.execute(translated, params or ())
        except psycopg2.errors.UndefinedColumn:
            # Swallow ONLY for DDL (e.g. ALTER TABLE ADD COLUMN that already exists).
            # For SELECT/INSERT/UPDATE, UndefinedColumn is a real schema bug —
            # re-raise so the caller sees it instead of silently returning [] from fetchall().
            self._conn._raw.rollback()
            if _is_ddl:
                return
            raise
        except psycopg2.errors.DuplicateColumn:
            # Always a DDL concern (ADD COLUMN on existing column) — safe to swallow.
            self._conn._raw.rollback()
            return
        except psycopg2.errors.UniqueViolation:
            self._conn._raw.rollback()
            raise  # caller must handle — silent swallow leaves lastrowid=None
        except psycopg2.errors.UndefinedTable:
            # Swallow only for DDL; re-raise for SELECT/DML so bugs surface.
            self._conn._raw.rollback()
            if _is_ddl:
                return
            raise
...
def init_db():
    ...
    print("[VERSION] db_pg.py = v7-table-pk-complete-2026-05-12", flush=True)
