...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt_is(txt, "menu_accounts"):
        ...
        print(f"[ACCOUNTS-MENU] total={total} rows_fetched={len(rows)}", flush=True)
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ACC:"):
        ...
        # ── Single CTE query: COUNT + rows in one round-trip ──────────────────
        # First pass: get total count only (offset=0) so we can clamp offset,
        # then fetch the actual page. Two small queries > one complex CTE when
        # offset clamping is needed before the LIMIT/OFFSET fetch.
        # But we use a subquery in SELECT to get total_count alongside rows,
        # avoiding any cursor/transaction state issues with thread-local conn.
        con = db()
        cur = con.cursor()
        try:
            cur.execute(f"""
        ...
        # total_count comes from window function — present in every row
        if not rows:
            # offset might be beyond range — try offset=0 once
            if offset > 0:
                con2 = db()
                cur2 = con2.cursor()
                try:
        ...
        total = int(rows[0]["total_count"])
        print(f"[ACCOUNTS] total={total} rows_fetched={len(rows)} offset={offset}", flush=True)
...
def main():
    ...
    print("[VERSION] main.py = v3-accounts-fix-2026-05-12", flush=True)
