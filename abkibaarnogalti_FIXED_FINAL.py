...
"Send your BEP-20 wallet address now:",
    ...
    if txt == "🧾 Balance history":
        ...
        lines = []
        for r in rows:
            upi_or_qr = r.get("upi_or_qr") or ""
            snip = upi_or_qr[:18] + ("..." if len(upi_or_qr) > 18 else "")
            method = (r.get("method") or "upi").lower()
            amt_inr = float(r.get("amount") or 0)
            amt_usd = float(r.get("amount_usd") or 0.0)
            if method == "crypto":
                if amt_usd == 0:
                    amt_usd = inr_to_usd(amt_inr)
    ...
    if txt == "💳 PAYOUT REQUEST":
        # Enter admin payout type menu (UPI 🚀 vs CRYPTO)
...
def _admin_ev_set_verified(cur, action_id: int, admin_id: int):
    ...
    # Same transaction / same cursor to avoid sqlite lock
    cur.execute("UPDATE actions SET state=?, updated_at=? WHERE action_id=?", ("approved", now_ts, int(action_id)))

    cur.execute("SELECT reg_id, user_id FROM actions WHERE action_id=?", (int(action_id),))
    a = cur.fetchone()
    if a:
        cur.execute("UPDATE registrations SET state=? WHERE id=?", ("approved", int(a["reg_id"])))
            ...
            if c >= 10:
                ...
                if not already:
                    cur.execute(
                        ...
                        (int(ref_id), int(a["user_id"]), 10.0, now_ts),
                    )
    ...
    cur.execute(
        ...
        (int(action_id), int(admin_id), "VERIFIED", None, now_ts),
    )

def _admin_ev_set_not_verified(cur, action_id: int, admin_id: int, reason: str):
    now_ts = int(time.time())
    # Revert provisional HOLD credit (same transaction / same cursor)
    ...
    if a:
        ...
        if pc and int(pc["reverted"] or 0) == 0:
            # Manually revert to avoid nested DB writes/locks
            cur.execute("UPDATE users SET hold_balance = hold_balance - ? WHERE user_id=?", (float(pc["amount"]), int(a["user_id"])))
            cur.execute(
                "INSERT INTO ledger(user_id, kind, amount, note, created_at) VALUES(?,?,?,?,?)",
                (int(a["user_id"]), "hold", -float(pc["amount"]), f"Revert precredit for action {int(action_id)}", now_ts),
            )
            cur.execute("UPDATE precredits SET reverted=1 WHERE action_id=?", (int(action_id),))

        cur.execute("UPDATE actions SET state=?, updated_at=? WHERE action_id=?", ("rejected", now_ts, int(action_id)))
        cur.execute("UPDATE registrations SET state=? WHERE id=?", ("rejected", int(a["reg_id"])))

    cur.execute(
        "INSERT OR REPLACE INTO admin_email_verify(action_id, decided_by, status, reason, decided_at) VALUES(?,?,?,?,?)",
        (int(action_id), int(admin_id), "NOT_VERIFIED", str(reason), now_ts),
    )
...
# C) Payout amount selection
    if data.startswith("PAY_AMT:"):
        ...
        context.user_data["payout_reply_mode"] = "upi"
        await q.message.reply_text("PLEASE ENTER YOUR UPI ID OR QR CODE", reply_markup=payout_selected_kb("1. UPI 🚀"))
        return
...
async def upi_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if kind == "upi":
        if not (upi.lower().startswith("upi://") or is_valid_upi_id(upi)):
            await update.message.reply_text("❌ INVALID UPI ID. Example: name@bank", reply_markup=payout_selected_kb("1. UPI 🚀"))
            return
