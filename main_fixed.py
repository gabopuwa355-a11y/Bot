...
def main():
    ...
    # Migration: jo users ne kabhi lang set nahi ki, unka 'hi' default NULL kar do
    # Taaki wo /start pe English menu dekhein (neutral default)
    try:
        con = db()
        cur = con.cursor()
        cur.execute("UPDATE users SET lang=NULL WHERE lang='hi'")
        con.commit()
        con.close()
        print("[MIGRATION] lang 'hi' → NULL for users who never set language")
    except Exception as e:
        print(f"[MIGRATION] lang reset failed (ok): {e}")
