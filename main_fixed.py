...
Send your BEP-20 wallet address now:""",reply_markup=back_only_menu())
    ...
    if txt == "🧾 Balance history":
        # Mixed history (Hold/Main movement + payout requests) in one view
        await update.message.reply_text(build_balance_history_text(user.id, limit_ledger=10, limit_payouts=5))
        return
...
async def upi_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute(
        "INSERT INTO payouts(user_id, amount, amount_usd, method, upi_or_qr, meta, created_at, state, reserved, refunded) VALUES(?,?,?,?,?,?,?,?,?,?)",
        (user.id, inr_need, 0.0, 'upi', (upi.lower() if kind=="upi" else upi), '', now, "pending", 1, 0)
    )
    pid = cur.lastrowid
    con.commit()
    con.close()
