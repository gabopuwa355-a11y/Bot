...
# =========================
# AUTOMATION MODE (Railway friendly: bot <-> userbot bridge via ADMIN chat)
# =========================
AUTOMATION_MODE = (os.environ.get("AUTOMATION_MODE") or "1").strip().lower() in ("1", "true", "yes", "on")
AUTOMATION_DONE_LABEL = "✔️ Done"
AUTOMATION_REQUEST_QUEUE = []   # FIFO user ids waiting for a fresh task
AUTOMATION_DONE_QUEUE = []      # FIFO user ids waiting for Done result
AUTOMATION_USER_TASK_TIME = {}  # user_id -> task assigned ts
AUTOMATION_USER_ACTIVE = {}     # user_id -> last task text

def automation_task_menu() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup([[AUTOMATION_DONE_LABEL]], resize_keyboard=True)

def automation_convert_price(text: str) -> str:
    s = str(text or "")
    return s.replace("0.145$", "₹08").replace("0.17$", "₹10")

def automation_parse_admin_payload(text: str):
    # Supported inbound formats from userbot/admin:
    #   AUTO|TASK|<user_id>|<task text>
    #   AUTO|RESULT|<user_id>|SUCCESS|<message>
    #   AUTO|RESULT|<user_id>|FAIL|<message>
    # Fallbacks are handled elsewhere for plain text / ✅ / ❌ messages.
    raw = str(text or "")
    if not raw.startswith("AUTO|"):
        return None
    parts = raw.split("|", 4)
    if len(parts) < 3:
        return None
    ...
    return {"kind": kind, "user_id": uid, "status": status, "payload": payload}
def smart_format_task(text: str) -> str:
    if not text:
        return text

    text = text.replace("0.145$", "₹08").replace("0.17$", "₹10")
    ...
    for line in lines:
        ...
        if line.lower().startswith("first name:"):
            ...
            formatted.append(f"👤 First name:\n`{val}`")

        elif line.lower().startswith("last name:"):
            val = line.split(":",1)[1].strip()
            formatted.append(f"📛 Last name:\n`{val}`")
            ...
            val = line.split(":",1)[1].strip()
            formatted.append(f"🔑 Password:\n`{val}`")

        elif "recovery email" in line.lower():
            email = line.split()[-1]
            formatted.append(f"🔐 Recovery email:\n`{email}`")

        else:
            formatted.append(line)

    return "\n\n".join(formatted)
async def automation_send_signal(context: ContextTypes.DEFAULT_TYPE, kind: str, user_id: int):
    # Outbound signal for userbot. Userbot should listen to admin account's bot chat.
    try:
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"AUTO|{kind.upper()}|{int(user_id)}|{int(time.time())}")
    except Exception as e:
        print("AUTOMATION SIGNAL ERROR:", e)

async def automation_assign_task(context: ContextTypes.DEFAULT_TYPE, user_id: int, task_text: str):

    # 🔥 NEW FORMAT APPLY
    task_text = smart_format_task(task_text)

    AUTOMATION_USER_TASK_TIME[int(user_id)] = int(time.time())
    AUTOMATION_USER_ACTIVE[int(user_id)] = task_text

    await context.bot.send_message(
        chat_id=int(user_id),
        text=task_text,
        parse_mode="Markdown",   # 🔥 IMPORTANT (copy enable)
        reply_markup=automation_task_menu()
    )
async def automation_send_result(context: ContextTypes.DEFAULT_TYPE, user_id: int, ok: bool, message_text: str = ""):
    uid = int(user_id)
    msg = str(message_text or "").strip()
    if ok:
        AUTOMATION_USER_TASK_TIME.pop(uid, None)
        AUTOMATION_USER_ACTIVE.pop(uid, None)
        if not msg:
            msg = "✅ Task Completed"
        await context.bot.send_message(chat_id=uid, text=msg, reply_markup=main_menu_markup(uid))
    else:
        if not msg:
            msg = "❌ Task Not Completed"
        await context.bot.send_message(chat_id=uid, text=msg + "\n\n🔁 Try again", reply_markup=automation_task_menu())

async def handle_automation_admin_message(update: Update, context: ContextTypes.DEFAULT_TYPE, txt: str) -> bool:
    user = update.effective_user
    if not user or int(user.id) != int(ADMIN_ID) or not AUTOMATION_MODE:
        return False
    ...
    parsed = automation_parse_admin_payload(raw)
    if parsed:
        if parsed["kind"] == "TASK" and parsed.get("user_id"):
            await automation_assign_task(context, parsed["user_id"], parsed.get("payload") or "")
            return True
        if parsed["kind"] == "RESULT" and parsed.get("user_id"):
            ok = parsed.get("status") in ("OK", "SUCCESS", "DONE", "PASS")
            await automation_send_result(context, parsed["user_id"], ok, parsed.get("payload") or ("✅ Task Completed" if ok else "❌ Task Not Completed"))
            return True

    # Plain fallback result routing
    lowered = raw.lower()
    if raw.startswith("✅") or raw.startswith("❌") or "task completed" in lowered or "task not completed" in lowered:
        if AUTOMATION_DONE_QUEUE:
            target_uid = AUTOMATION_DONE_QUEUE.pop(0)
            ok = raw.startswith("✅") or "task completed" in lowered
            await automation_send_result(context, target_uid, ok, automation_convert_price(raw))
            return True

    # Plain fallback task routing
    if AUTOMATION_REQUEST_QUEUE:
        target_uid = AUTOMATION_REQUEST_QUEUE.pop(0)
        await automation_assign_task(context, target_uid, raw)
        return True
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt_is(txt, "menu_register"):
        ...
        if AUTOMATION_MODE:
            AUTOMATION_REQUEST_QUEUE.append(int(user.id))
            try:
                await automation_send_signal(context, "FETCH", int(user.id))
            except Exception:
                pass
            await update.message.reply_text("⏳ Fetching task...", reply_markup=main_menu_markup(user.id))
            return
    ...
    if AUTOMATION_MODE and (txt == AUTOMATION_DONE_LABEL or txt.lower() == "done" or "done" in txt.lower()):
        now = int(time.time())
        task_ts = int(AUTOMATION_USER_TASK_TIME.get(int(user.id), 0) or 0)
        if not task_ts:
            await update.message.reply_text("❌ No active task", reply_markup=main_menu_markup(user.id))
            return
        if now - task_ts > 22 * 3600:
            AUTOMATION_USER_TASK_TIME.pop(int(user.id), None)
            AUTOMATION_USER_ACTIVE.pop(int(user.id), None)
            await update.message.reply_text("⏰ Task expired", reply_markup=main_menu_markup(user.id))
            return
        ...
        try:
            ...
            pass
        await update.message.reply_text("⏳ Checking...", reply_markup=automation_task_menu())
        return
...
async def admin_content_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await handle_automation_admin_message(update, context, txt):
        return
